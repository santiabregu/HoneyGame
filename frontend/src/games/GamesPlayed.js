...
export default function GamesInProgress() {
  ...
  const fetchGames = async () => {
    try {
      const response = await fetch(`/api/v1/game/status/FINISHED`, {
        headers: {
  ...
  const tiempoPartida = (game) => {
    const diferencia = new Date(game.finish) - new Date(game.start);
    const totalEnSeg = Math.floor(diferencia / 1000);
    const minutos = Math.floor(totalEnSeg / 60);
    const seg = totalEnSeg % 60;
    return (
      minutos === 0 ? <strong> Time: {seg} seconds</strong> :
      <p><strong> Time: </strong>{minutos}:{seg}</p>
    )
  }
  return (
    <div className="games-played-container">
      <h1 className="text-center">Games played</h1>
      {modal}
      {games.length > 0 ? (
            ...
            <div key={game.id} className="game-card">
              ...
              <div className='game-body-container'>
                ...
                <div className='buttons-container'>
                  <span className="game-mode"> Gamemode: {game.gamemode}</span>
                  <span className="time-match">{tiempoPartida(game)} </span>
                </div>
              </div>
            </div>
          ))}
